...
6. **Compact shapes.** `CompanySummary`, `AssetSummary`, `ArticleSummary`, `WebsiteSummary`, `AssetPasswordSummary` (+ any other helper-bearing resource) are declared in `src/types/common.ts` next to `OperationMetadata`, `DryRunResult<T>`, `Resolution<T>`, `FieldDiff`. Helpers return the compact shape by default with `expand: true`; primitives keep returning the full record. Compact shapes never drop `id`/`name`/the field a caller resolves by.
